...
function NewValueDialog({ onClose, type, selectedPath, onSave }) {
  const [name, setName] = useState('New Value #1');
  const [data, setData] = useState(type === 'REG_DWORD' ? '0' : '');
  ...
  const handleSave = useCallback(() => {
    ...
    let saveData = data;
    if (type === 'REG_DWORD') {
      saveData = parseInt(data, 10) || 0;
    }
    onSave?.(selectedPath, name.trim(), type, saveData);
    onClose?.();
  }, [name, data, type, selectedPath, onSave, onClose]);
