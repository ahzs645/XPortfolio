...
export class WindowTemplates {
  ...
  static createIframeContainer(appPath, windowId, programConfig) {
    ...
    if (cacheAllowed && window.__IFRAME_CACHE && window.__IFRAME_CACHE.has(windowId)) {
      ...
      if (cachedData.fullyLoaded && cachedData.iframe) {
        ...
        const attrs = {
          ...
          sandbox:
            'allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox allow-top-navigation-by-user-activation allow-modals allow-downloads',
        };
        for (const [attr, value] of Object.entries(attrs)) iframe.setAttribute(attr, value);
        iframe.src = appPath;
        window.__IFRAME_CACHE.set(windowId, { ...cachedData, iframe });
    ...
    const prepareIframeForMobile = (targetIframe, { skipHide = false } = {}) => {
      const apply = () => {
        try {
      ...
      apply();
      try {
        targetIframe.addEventListener('load', apply, { once: true });
      } catch (_) {
    ...
    prepareIframeForMobile(iframe, { skipHide: skipHideForProgram });
...
function getPresetPosition(
  ...
  const presets = {
    ...
    'top-right': { x: viewportWidth - windowWidth - margin, y: margin },
    'bottom-left': { x: margin, y: viewportHeight - windowHeight - taskbarHeight - margin },
    'bottom-right': {
      x: viewportWidth - windowWidth - margin,
      y: viewportHeight - windowHeight - taskbarHeight - margin,
    },
    'center-left': { x: margin, y: (viewportHeight - windowHeight - taskbarHeight) / 2 },
    'center-right': {
      x: viewportWidth - windowWidth - margin,
      y: (viewportHeight - windowHeight - taskbarHeight) / 2,
    },
...
export function makeDraggable(manager, windowElement, handleElement) {
  ...
  const activateDrag = () => {
    ...
    dragActivated = true;
    windowElement.dispatchEvent(new CustomEvent('window-drag-start', { bubbles: false }));
  ...
  resizers.forEach((resizer) => {
    resizer.addEventListener('mousedown', (e) => {
      ...
      const onMouseUp = () => {
        ...
        if (iframe && iframe.contentWindow) {
          try {
            iframe.contentWindow.postMessage({ type: 'window-resize-end' }, '*');
          } catch (_) {
...
export function minimizeWindow(manager, windowElement, EVENTS) {
  ...
  windowElement.addEventListener('animationend', function handler(e) {
    if (e.animationName === 'windowMinimizeZoom') {
      ...
      if (iframe && iframe.contentWindow) {
        try {
          iframe.contentWindow.postMessage({ type: 'window:minimized' }, '*');
        } catch (_) {
          /* minimized postMessage failed */
        }
      }
      (window.batchedPublish || manager.eventBus.publish).call(
        ...
        { windowId: windowElement.id, __coalesce: true },
      );
...
export function toggleMaximize(manager, windowElement, _EVENTS) {
  ...
  if (!state.isMaximized) {
    ...
    const { width: vw, height: vh } = manager._getViewportDimensions();
...
export function deactivateAllWindows(manager, excludeWindow = null) {
  Object.values(manager.windows).forEach((win) => {
    if (win !== excludeWindow) {
      ...
      if (iframe && iframe.contentWindow) {
        try {
          iframe.contentWindow.postMessage({ type: 'window:blurred' }, '*');
        } catch (_) {
