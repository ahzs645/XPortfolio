...
class LegoCacheStrategy extends Strategy {
    async _handle(request, handler) {
        ...
        const cachedResponse = await caches.match(cacheKeyRequest, { ignoreVary: true });
...
async function checkCacheStatus(language, hdTextures, siFiles, client) {
    ...
    const cachedResults = await Promise.all(
        requiredFiles.map(async (file) => {
            const response = await cache.match(file, { ignoreVary: true });
            return response && !isHtmlFallback(response);
        })
    );
    const missingFiles = requiredFiles.filter((_, index) => !cachedResults[index]);
